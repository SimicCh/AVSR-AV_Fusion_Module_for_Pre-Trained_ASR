...
def load_video_moviepy(path):
    videogen = VideoFileClip(path)
    frames = np.array([frame for frame in videogen.iter_frames()])
    return frames
...
def write_video_moviepy(rois, target_path):
    fps = 25
    new_clip = ImageSequenceClip(list(rois), fps=fps)
    new_clip.write_videofile(target_path, logger=None) 
    return
...
def write_video_ffmpeg(rois, target_path, ffmpeg):
    ...
    cmd = [FFMPEG_BINARY, "-f", "concat", "-safe", "0", "-i", list_fn, "-q:v", "1", "-r", str(fps), '-y', '-crf', '20', target_path]
...
def crop_patch(video_pathname, landmarks, mean_face_landmarks, stablePntsIDs, STD_SIZE, window_margin, start_idx, stop_idx, crop_height, crop_width):
    ...
    frames_moviepy = load_video_moviepy(video_pathname)
    frame_gen = read_video(video_pathname)
    margin = min(num_frames, window_margin)
    while True:
        try:
            # frame = frame_gen.__next__() ## -- BGR
            frame = frames_moviepy[frame_idx]
